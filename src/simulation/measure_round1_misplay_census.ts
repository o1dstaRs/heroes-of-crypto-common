...
function round1Game(genome: ILeagueGenome, options: INormalizedCensusOptions, game: number): IMatchConfig {
    ...
    return {
        ...
        redRoster: leagueRoster(pick.state.right.creatures),
        seed: battleSeed,
        maxLaps: options.maxLaps,
        gridType: PBTypes.GridVals.NORMAL,
        greenArtifactT1: pick.state.left.tier1Artifact,
        redArtifactT1: pick.state.right.tier1Artifact,
        greenArtifactT2: pick.state.left.tier2Artifact,
        redArtifactT2: pick.state.right.tier2Artifact,
        greenDoctrine: pick.state.left.doctrine,
        redDoctrine: pick.state.right.doctrine,
        greenAugments: pick.lowerAugments,
        redAugments: pick.upperAugments,
        greenSynergies: SETUP_POLICY_V0.pickSynergies(pick.state.left.creatures),
        redSynergies: SETUP_POLICY_V0.pickSynergies(pick.state.right.creatures),
    };
}
