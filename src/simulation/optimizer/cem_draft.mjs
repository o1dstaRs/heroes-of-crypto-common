 ...
 * -----------------------------------------------------------------------------
 * Cross-Entropy Method trainer for the DRAFT policy weight vector (11-dim, creature_score.DRAFT_FEATURE_NAMES).
 ...
 * anchor in a `cemDraft` self-play tournament (cem_draft_eval.ts), fanned out as parallel subprocesses.
 *
...
const EVAL = join(HERE, "..", "cem_draft_eval.ts"); // src/simulation/cem_draft_eval.ts

// Must stay in sync with creature_score.DRAFT_ANCHOR_W (scoreCreature's coefficients).
const ANCHOR = [35, 0.125, 95, 3, 1.2, 5, 6, 50, 70, 60, 45];
...
const OUT = join(process.cwd(), "sim-out", "cem_draft");
...
async function main() {
    writeFileSync(LOG, "");
    log(
        `CEM draft: DIM=${DIM} POP=${POP} ELITE=${ELITE} GENS=${GENS} GAMES=${GAMES} CONC=${CONC} ` +
