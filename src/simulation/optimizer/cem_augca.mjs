 ...
 * -----------------------------------------------------------------------------
 * CEM trainer for the composition + OPPONENT-aware AUGMENT policy (20-dim = 4 kinds x 5 features). Mirrors
 ...
 * the frozen blind heuristic in a `cemAugCA` self-play tournament (cem_augca_eval.ts), fanned out as subprocesses.
 *
...
const EVAL = join(HERE, "..", "cem_augca_eval.ts");

const DIM = 20; // 4 augment kinds x 5 features
...
const OUT = join(process.cwd(), "sim-out", "cem_augca");
...
const evaluate = (weights, games, seed) =>
    new Promise((resolve) => {
        const child = spawn("bun", [EVAL, String(games), String(seed), String(CONC)], {
            env: { ...process.env, V05_AUGCA_WEIGHTS: JSON.stringify(weights) },
            cwd: process.cwd(),
        });
        let out = "";
...
async function main() {
    writeFileSync(LOG, "");
    log(`CEM augCA: DIM=${DIM} POP=${POP} ELITE=${ELITE} GENS=${GENS} GAMES=${GAMES} CONC=${CONC} sigma0=${SIGMA0} hours=${HOURS} -> ${OUT}`);
    let bestVal = -1;
