...
const normalizeCohorts = (summary: UnknownRecord, rows: readonly INormalizedRow[]): INormalizedCohort[] => {
    ...
    input.forEach((value, index) => {
        if (typeof value === "string" || typeof value === "number") {
            ...
            if (id && id !== "all") output.set(id, { id, label: titleCase(id), description: "", games: null });
            return;
        }
...
export function renderAiMetaReport(summaryValue: unknown, options: IRenderAiMetaReportOptions = {}): string {
    ...
    const fightVersion = textValue(asRecord(summary.provenance), ["fightVersion", "aiVersion"], "unknown");
...
@media(max-width:720px){.shell{width:min(100% - 24px,1260px)}.hero-inner{padding:34px 0 52px}.brand{margin-bottom:24px}.hero-stats{grid-template-columns:repeat(2,1fr)}.notice{grid-template-columns:1fr}.notice-mark{display:none}.leaders{grid-template-columns:repeat(2,1fr)}.forest-row{grid-template-columns:minmax(110px,150px) minmax(100px,1fr) 54px}.table-tools{grid-template-columns:1fr}.details-grid{grid-template-columns:1fr}.section-head{display:block}.section-head p{margin-top:7px}.footer{display:block}.footer span{display:block;margin-top:5px}}
...
<header class="hero"${heroStyle}>
  <div class="shell hero-inner">
    <div>
      ...
      <p class="subtitle">Units, artifacts, and augment loadouts ranked by draw-aware score rate—with uncertainty and selection frequency kept visible.</p>
    </div>
...
<main class="shell report-main">
  <aside class="notice" aria-label="Interpretation caveat">
    ...
    <div><h2>Strength comes from controlled exploration</h2><p>Artifact and augment score rates use the policy's 20% uniform exploration assignments, breaking the link between a strong army and its usual item choice. Pick rate is the overall selection frequency under the 80/20 policy. Augment plans are the causal unit; individual levels remain compositional diagnostics.</p></div>
  </aside>
  <div class="sticky-filter"><nav class="cohort-tabs" id="cohort-tabs" aria-label="Filter report by cohort"></nav></div>

  <section aria-labelledby="leaders-title">
    <div class="section-head"><div><p class="eyebrow">At a glance</p><h2 id="leaders-title">Category leaders</h2></div><p>Highest observed draw-aware score rate in the selected cohort. Treat close results as a tier rather than a proven unique winner: confidence intervals can overlap, and the augment-plan view compares 96 candidates.</p></div>
    <div class="leaders" id="leaders"></div>
  </section>

  <section aria-labelledby="rankings-title">
    <div class="section-head"><div><p class="eyebrow">Comparative performance</p><h2 id="rankings-title">Score-rate forest plots</h2></div><p>The gold rule marks 50%. Whiskers use reported 95% intervals; when only binary outcomes are available, a Wilson interval is reconstructed.</p></div>
  ...
  <section aria-labelledby="table-title">
    <div class="section-head"><div><p class="eyebrow">Audit the numbers</p><h2 id="table-title">Full rankings</h2></div><p>Search any unit or item, restrict by category, and sort on every reported metric.</p></div>
...
var state={cohort:"all",query:"",type:"all",sort:"rate",direction:-1};
...
function renderTable(){var body=document.getElementById("ranking-body");var values=tableRows();body.replaceChildren();values.forEach(function(row){var line=node("tr");line.append(td(row.categoryLabel));var entry=td("");var wrap=node("div","table-entry");var img=node("img");img.src=image(row);img.alt="";var copy=node("div");copy.append(node("strong","",row.name));copy.append(node("span","type-chip",row.key));wrap.append(img,copy);entry.append(wrap);line.append(entry);line.append(td(cohortLabel(row.cohort)));line.append(td(sample(row)));line.append(td(finite(row.wins)?numberFormat.format(row.wins):"—"));line.append(td(finite(row.losses)?numberFormat.format(row.losses):"—"));line.append(td(finite(row.draws)?numberFormat.format(row.draws):"—"));line.append(td(rate(row.scoreRate),"rate-cell"));line.append(td(rate(row.winRate)));line.append(td(finite(row.ciLow)&&finite(row.ciHigh)?rate(row.ciLow)+" – "+rate(row.ciHigh):"—"));line.append(td(rate(row.pickRate)));line.append(td(lift(row.liftPp),tone(row.liftPp)));body.append(line)});if(!values.length){var line=node("tr");var empty=td("No ranking rows match these filters.","empty");empty.colSpan=12;line.append(empty);body.append(line)}document.getElementById("table-total").textContent=numberFormat.format(values.length)+" rows"}
function bindTable(){var search=document.getElementById("table-search");search.addEventListener("input",function(){state.query=search.value.trim();renderTable()});var select=document.getElementById("type-filter");select.addEventListener("change",function(){state.type=select.value;renderTable()});document.querySelectorAll("[data-sort]").forEach(function(button){button.addEventListener("click",function(){var key=button.getAttribute("data-sort");if(state.sort===key)state.direction*=-1;else{state.sort=key;state.direction=key==="name"||key==="categoryLabel"||key==="cohort"?1:-1}renderTable()})})}
function renderAll(){renderTabs();renderLeaders();renderForests();renderScatter();renderHeatmap();renderTable()}
renderTypeFilter();bindTable();renderAll();
})();
</script>
</body>
</html>`;
