...
import {
    evaluateRankedDraftInteractionPriorGates,
    interactionPriorEntrants,
} from "../../src/simulation/measure_ranked_draft_interaction_prior";
...
describe("ranked draft interaction-prior acceptance gate", () => {
    test("requires a strictly winning draw-aware average and confidence support", () => {
        const pass = evaluateRankedDraftInteractionPriorGates(report(5, 3, 0, 0.51));
        expect(pass.verdict).toBe("PASS");
        expect(pass.headToHead.drawAwareScore).toBe(0.625);

        const tiedAverage = evaluateRankedDraftInteractionPriorGates(report(4, 4, 0, 0.51));
        expect(tiedAverage.verdict).toBe("FAIL");
        expect(tiedAverage.gates.find((gate) => gate.name === "draw_aware_head_to_head")?.passed).toBe(false);

        const weakConfidence = evaluateRankedDraftInteractionPriorGates(report(5, 3, 0, 0.5));
