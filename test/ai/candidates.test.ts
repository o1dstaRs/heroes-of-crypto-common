...
describe("candidates — the F4 enumerated candidate generator", () => {
    ...
    it("move-shots: never generated whatever the cap or discovery asks — only melee may follow a move", () => {
        const c = createCombatTestContext();
        const shooter = createTestUnit({
            team: LEFT,
        ...
        // This position used to yield up to two move-then-shot composites once opted in; the class is retired.
        for (const options of [
            { maxMoveShotComposites: 1 },
            { maxMoveShotComposites: 2, discoverMoveShotTargetsAfterMove: true },
            { maxMoveShotComposites: 99 },
        ]) {
            const enabled = enumerateCandidates(shooter, context, incumbent, options).candidates;
            expect(enabled.some(hasMoveAndShot)).toBe(false);
