...
describe("v0.5 — reinforcement-learned strategy", () => {
    ...
    it("ships the long-run-trained vector (24 dims, ~55.9% vs v0.4 on fresh seeds)", () => {
        // Panel-validated pass-2 best (~55.9%) + 2 neutral stage-6 levers ([22] stand-support, [23] wounded).
        expect(DEFAULT_V05_W).toEqual([
            0.878, -0.322, -0.1112, 0.1731, 3.2121, 2.866, 1.0165, 0.4742, 0.2352, 2.4026, 0.3742, 0.9864, 0.5068,
            0.8132, 0.0245, 1.0611, 1.9781, 0.61, -0.651, 0.9425, -2.0235, 0.6172, 0.0, 0.0,
        ]);
        expect(DEFAULT_V05_W.length).toBe(V05_WEIGHT_KEYS.length);
        expect(DEFAULT_V05_W.length).toBe(24);
    });
