...
describe("ranked Chaos MOVEMENT synergy folds into getSteps via the server refresh", () => {
    it("refreshStackPowerForAllUnits applies the team's movement synergy to getSteps()", () => {
        ...
        const mover = createTestUnit({ name: "Beholder", team: LEFT, attackType: RANGE, initiative: 4, morale: 0 });
        c.grid.occupyCell({ x: 6, y: 1 }, mover.getId(), LEFT, 1, false, false);
        ...
        fp.setSynergyUnitsPerFactions(LEFT, 0, 2, 0, 0);
        const applied = fp.updateSynergyPerTeam(LEFT, CHAOS, ChaosSynergy.MOVEMENT, 1);
        expect(applied).toBe(true);
        expect(fp.getAdditionalMovementStepsPerTeam(LEFT)).toBeGreaterThan(0);

        // The server now runs this refresh before every reachable-set check.
        c.unitsHolder.refreshStackPowerForAllUnits();
        expect(mover.getSteps()).toBe(base + fp.getAdditionalMovementStepsPerTeam(LEFT));
