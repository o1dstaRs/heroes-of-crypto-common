...
/**
 * Strict six-capture aggregation gate for the A13 melee target-layer optimization.
 *
 * This runner never executes a match. It accepts exactly the six independently sealed pair reports
 * preregistered in ../a13_melee_layers_replication_protocol_2026-07-22.json, verifies every source,
 ...
 *
 * Example:
 *   bun docs/evidence/tools/a13_melee_layers_replication.ts \
 *     --r0=/tmp/r0.json --r1=/tmp/r1.json --r2=/tmp/r2.json \
 *     --r3=/tmp/r3.json --r4=/tmp/r4.json --r5=/tmp/r5.json \
 *     --out=/tmp/a13-melee-layers-replication.json
 */
...
const SCHEMA = "heroes-of-crypto/a13-melee-layers-replication/v1" as const;
const CAPTURE_SCHEMA = "heroes-of-crypto/a13-melee-layers-pair/v1" as const;
const PROTOCOL_SCHEMA = "heroes-of-crypto/a13-melee-layers-replication-protocol/v1" as const;
const PROTOCOL_DATE = "2026-07-22" as const;
const BASELINE_COMMIT = "f24a78d985d30124e5d2e7bfa6ed30eca52de907";
const BASELINE_SRC_MANIFEST_SHA256 = "11083c9077d1ab166fd4e40531a251876b8d44b6fb0d07dc9a03ffbdad60ddd9";
const BASELINE_AI_SHA256 = "115f5ffc48006daa2ab7adb4da778acaf53cc2a47ecece080dd8119090afccbd";
const CANDIDATE_SRC_MANIFEST_SHA256 = "505b84481c0407a4fac3698e1484925df27419b5ef49ca964c8845766df8b1c6";
const CANDIDATE_AI_SHA256 = "02f56c80806b28b29e393c07d77a83ebeb532b780440bc59897673b8268efe7d";
const CANDIDATE_MODULE_SHA256 = "18114a5eb7205de721b2cd6788445b0c8ac352759d017b112813e177a9ebe069";
const SUPERSEDED_REPORT_SHA256 = "ef4c585d4d14c82cd28799cea15a7eec7e919a6b85ea626010341deff51b5834";
...
const PROTOCOL_PATH = resolve(dirname(RUNNER_PATH), "../a13_melee_layers_replication_protocol_2026-07-22.json");
...
function printHelp(): void {
    console.log(`A13 melee target-layer six-capture replication gate
...
function loadProtocol(): { protocol: IProtocol; seal: IFileSeal } {
    ...
    const superseded = requireRecord(raw.supersededEvidence, "protocol.supersededEvidence");
    assertEqual(superseded.reportSha256, SUPERSEDED_REPORT_SHA256, "superseded report hash");
    assertEqual(superseded.disposition, "superseded-not-pooled", "superseded report disposition");
...
function validateSource(
    ...
    if (differences.length !== EXPECTED_RUNTIME_DELTA.length) {
        throw new Error(`${captureId} must contain exactly two source differences`);
    }
...
function validateCapture(
    ...
    if (file.sha256 === SUPERSEDED_REPORT_SHA256) {
        throw new Error(`${id} is the superseded historical report and must not be pooled`);
    }
...
async function main(): Promise<void> {
    ...
    const report = {
        ...
        protocol: {
            ...
            supersededEvidence: loadedProtocol.protocol.supersededEvidence,
        },
        aggregationRunner: {
            before: runnerBefore,
            after: runnerAfter,
