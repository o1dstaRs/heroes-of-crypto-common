...
const SCHEMA = "heroes-of-crypto/a13-stat-rounding-near-grid-replication/v3" as const;
const CAPTURE_SCHEMA = "heroes-of-crypto/a13-stat-rounding-near-grid-pair/v3" as const;
const PROTOCOL_SCHEMA = "heroes-of-crypto/a13-stat-rounding-near-grid-replication-protocol/v3" as const;
const MICRO_SCHEMA = "heroes-of-crypto/a13-stat-rounding-near-grid-micro/v4" as const;
...
const LEDGER_SCHEMA = "heroes-of-crypto/a13-stat-rounding-near-grid-attempt-ledger/v3" as const;
const CAMPAIGN_SCHEMA = "heroes-of-crypto/a13-stat-rounding-near-grid-campaign/v2" as const;
const CAMPAIGN_ID = "a13-stat-rounding-near-grid-77ee4616-20260723-v5" as const;
...
const CAPTURE_RUNNER_SHA256 = "2ac2cfb84b3ab74507cdf1e3cc730b947a54de95cbe1ae6cf2b8a9d5ad1ead57";
const MICRO_RUNNER_SHA256 = "40c553be78d41b955a21e448e3e9d9017fea7f5dd21f819c688f7ab18823f740";
...
const PROTOCOL_PATH = resolve(
    dirname(RUNNER_PATH),
    "../a13_stat_rounding_near_grid_v5_replication_protocol_2026-07-23.json",
);
const PAIR_RUNNER_PATH = resolve(dirname(RUNNER_PATH), "a13_stat_rounding_near_grid_v5_pair.ts");
const MICRO_RUNNER_PATH = resolve(dirname(RUNNER_PATH), "a13_stat_rounding_near_grid_v5_micro.ts");
const PROFILE_RUNNER_PATH = resolve(dirname(RUNNER_PATH), "a13_stat_rounding_near_grid_v5_profile.ts");
const CAMPAIGN_RUNNER_PATH = resolve(dirname(RUNNER_PATH), "a13_stat_rounding_near_grid_v5_campaign.ts");
...
const INCIDENT_PATH = resolve(
    dirname(RUNNER_PATH),
    "../a13_stat_rounding_near_grid_v4_infrastructure_incident_2026-07-23.json",
);
...
const EXPECTED_DEVELOPMENT_PROVENANCE = Object.freeze({
    ...
    freshEvidencePolicy: {
        freshV5TimingsOnly: true,
        predecessorTimingReadByV5Runners: false,
...
const EXPECTED_OVERLAP_MARKERS = Object.freeze([
    "a13_stat_rounding",
    "a13_stat_rounding_near_grid_v5_pair",
    "a13_stat_rounding_near_grid_v5_micro",
    "a13_stat_rounding_near_grid_v5_profile",
    "a13_stat_rounding_near_grid_v5_replication",
    "a13_stat_rounding_near_grid_v5_campaign",
    "run_tournament.ts",
    "run_match.ts",
...
function commandLine(): ICli | undefined {
    ...
    if (values.help) {
        console.log(
            "Usage: bun docs/evidence/tools/a13_stat_rounding_near_grid_v5_replication.ts " +
...
function loadProtocol(): { value: Record<string, unknown>; seal: IFileSeal } {
    ...
    assertEqual(
        value.sourceDependencyPreflight,
        {
            schema: "heroes-of-crypto/a13-stat-rounding-near-grid-source-dependency-preflight/v1",
            executionBoundary:
            ...
            expectedRoots: {
                baseline: "/private/tmp/hoc-stat-rounding-near-grid-v5-qualification.eU9OEp/baseline",
                candidate: "/private/tmp/hoc-stat-rounding-near-grid-v5-qualification.eU9OEp/candidate",
            },
            sourceArchives: {
                baseline: {
                    path: "/private/tmp/hoc-stat-rounding-near-grid-v5-qualification.eU9OEp/baseline.tar",
                    sha256: BASELINE_ARCHIVE_SHA256,
                    srcTreeManifestSha256: BASELINE_SRC_MANIFEST_SHA256,
                },
                candidate: {
                    path: "/private/tmp/hoc-stat-rounding-near-grid-v5-qualification.eU9OEp/candidate.tar",
    ...
    assertEqual(
        value.v4InfrastructureIncident,
        {
            schema: "heroes-of-crypto/a13-stat-rounding-near-grid-v4-infrastructure-incident/v1",
            incident: {
                path: "docs/evidence/a13_stat_rounding_near_grid_v4_infrastructure_incident_2026-07-23.json",
                sha256: incidentSeal.sha256,
            },
            outcome: "infrastructure-invalid-before-semantic-or-timing",
            semanticArtifactProduced: false,
            performanceDataProduced: false,
            v4TimingEvidenceAcceptedOrPooled: false,
            v5Policy: "All semantic, micro, profile, and macro timing evidence must be freshly produced by v5.",
        },
        "protocol v4 infrastructure incident",
    );
...
function validateLedger(
    ...
    validateRecordedFileSeal(
        requireRecord(frozen.v4InfrastructureIncident, "ledger frozen v4 incident"),
        INCIDENT_PATH,
        "ledger frozen v4 incident",
    );
...
async function main(): Promise<void> {
    ...
    assertEqual(fileSeal(INCIDENT_PATH), incidentBefore, "v4 infrastructure incident pre/post");
