...
/**
 * Sealed two-source-root macro qualification for A13 Workstream 5 canonical first-layer elision.
 *
 * The runner compares complete source roots and accepts exactly this runtime delta:
 *
 *   modified  src/ai/ai.ts
 *   modified  src/ai/decision_path_catalog.ts
 *   modified  src/ai/internal/melee_target_layers.ts
 *
 ...
 *   bun docs/evidence/tools/a13_melee_first_layer_pair.ts \
 *     --baseline-root=/tmp/common-baseline \
 *     --candidate-root=/tmp/common-candidate \
 *     --out=/tmp/a13-melee-first-layer-pair.json
 *
 * Structural smoke:
 *   bun docs/evidence/tools/a13_melee_first_layer_pair.ts \
 *     --smoke \
 *     --baseline-root=/tmp/common-baseline \
 *     --candidate-root=/tmp/common-candidate \
 *     --out=/tmp/a13-melee-first-layer-pair-smoke.json
 */
...
const SCHEMA = "heroes-of-crypto/a13-melee-first-layer-pair/v1" as const;
...
const INHERITED_INPUT_ENVIRONMENT_KEYS = [
    "FORCE_CREATURES",
    "COHORT",
    "ROSTER_RANGED_MIN",
    "ROSTER_RANGED_MAX",
...
const EXACT_GOVERNED_ENVIRONMENT_KEYS = [
    "LIVETWIN",
    "FIGHT_MELEE_ROSTERS",
    ...INHERITED_INPUT_ENVIRONMENT_KEYS,
] as const;
...
type EnvironmentIsolationScope = "moduleLoads" | "searchSelectionChecks" | "rosterBuilds" | "matches";
...
interface IVariant {
    ...
    isolationAudit: IEnvironmentIsolationAudit;
}

interface IEnvironmentIsolationAudit {
    scopedCallbacks: Record<EnvironmentIsolationScope, number>;
    boundaryAssertions: number;
}
...
function printHelp(): void {
    console.log(`A13 canonical melee first-layer sealed paired macro qualification
...
function shouldScrubEnvironmentKey(key: string): boolean {
    return (
        ...
        EXACT_GOVERNED_ENVIRONMENT_KEYS.some((inputKey) => key === inputKey)
    );
}
...
function scrubExperimentEnvironment(): () => void {
    ...
    if (Object.keys(process.env).some(shouldScrubEnvironmentKey)) {
        throw new Error("Failed to remove inherited experiment and workload environment");
    }
    return (): void => {
        for (const key of Object.keys(process.env)) {
...
function assertIsolatedEnvironment(
    environment: Readonly<Record<string, string | undefined>>,
    audit: IEnvironmentIsolationAudit,
    scope: EnvironmentIsolationScope,
    boundary: "entry" | "exit",
): void {
    const governedKeys = new Set([
        ...Object.keys(environment).filter(shouldScrubEnvironmentKey),
        ...Object.keys(process.env).filter(shouldScrubEnvironmentKey),
        ...INHERITED_INPUT_ENVIRONMENT_KEYS,
    ]);
    for (const key of governedKeys) {
        if (process.env[key] !== environment[key]) {
            throw new Error(
                `Environment isolation failed at ${scope}:${boundary} for ${key}: ` +
                    `expected=${String(environment[key])} actual=${String(process.env[key])}`,
            );
        }
    }
    audit.boundaryAssertions++;
}
...
function withEnvironment<T>(
    ...
    try {
        audit.scopedCallbacks[scope]++;
        assertIsolatedEnvironment(environment, audit, scope, "entry");
        const result = callback();
        assertIsolatedEnvironment(environment, audit, scope, "exit");
        return result;
    } finally {
...
async function loadVariant(
    ...
    const [army, battle, productionSearch] = await withEnvironmentAsync(
        environment,
        isolationAudit,
        "moduleLoads",
        () =>
    ...
    const promoted = withEnvironment(environment, isolationAudit, "searchSelectionChecks", () =>
...
function run(
    ...
    const roster = withEnvironment(variant.environment, variant.isolationAudit, "rosterBuilds", () =>
        variant.army.buildRoster(variant.army.makeRng(seed)),
    );
    ...
    const result = withEnvironment(variant.environment, variant.isolationAudit, "matches", () =>
        variant.battle.runMatch(config),
    );
...
async function main(): Promise<void> {
    ...
    const isolationAudit: IEnvironmentIsolationAudit = {
        scopedCallbacks: {
            moduleLoads: 0,
            searchSelectionChecks: 0,
            rosterBuilds: 0,
            matches: 0,
        },
        boundaryAssertions: 0,
    };
    try {
        ...
        const baseline = await loadVariant("baseline", options.baselineRoot, isolationAudit);
        const candidate = await loadVariant("candidate", options.candidateRoot, isolationAudit);
        ...
        const report = {
            ...
            profile: {
                ...
                environmentIsolation: {
                    scrubbedPrefixes: SCRUBBED_ENVIRONMENT_PREFIXES,
                    exactGovernedKeys: EXACT_GOVERNED_ENVIRONMENT_KEYS,
                    fixedOverrides: environmentEntries(FIXED_ENVIRONMENT_OVERRIDES),
                    startupScrubVerified: true,
                    entryAndExitVerified: true,
                    inheritedValuesAbsentOrOverriddenAtEveryBoundary: true,
                    scopedCallbacks: isolationAudit.scopedCallbacks,
                    boundaryAssertions: isolationAudit.boundaryAssertions,
                },
                searchConstruction:
        ...
        if (eligible && !qualified) {
            throw new Error(
                `A13 canonical melee first-layer qualification gates failed; report retained at ${options.out}`,
            );
        }
    } finally {
        restoreEnvironment();
