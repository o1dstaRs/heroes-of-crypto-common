 ...
 *
 * Evidence:
 *   bun docs/evidence/tools/a13_stat_rounding_near_grid_v6_micro.ts \
...
const SCHEMA = "heroes-of-crypto/a13-stat-rounding-near-grid-micro/v5" as const;
const SOURCE_PREFLIGHT_SCHEMA = "heroes-of-crypto/a13-stat-rounding-near-grid-micro-source-preflight/v1" as const;
...
const EXPECTED_SOURCE_ENTRIES = 372;
const EXPECTED_SOURCE_BYTES = 10_011_554;
const EXPECTED_SOURCE_MANIFEST_SHA256 = "1532611d2da05f628b92f3e51101bfdd0149089deadad25836da7dc51d4d8b9f";
...
const MICRO_RUNTIME_DEPENDENCY_MANIFEST_CODEC =
    "heroes-of-crypto/a13-stat-rounding-near-grid-micro-manifest/type-tagged-v1" as const;
...
interface IDirectorySeal {
    ...
    entryCount: number;
    bytes: number;
    manifestCodec: typeof MICRO_RUNTIME_DEPENDENCY_MANIFEST_CODEC;
...
interface IRunSeal {
    ...
    gitTree: string;
    source: {
        manifestSchema: typeof A13_NEAR_GRID_SOURCE_MANIFEST_SCHEMA;
        manifestPathScope: typeof A13_NEAR_GRID_SOURCE_MANIFEST_PATH_SCOPE;
        manifestComparator: typeof A13_NEAR_GRID_SOURCE_MANIFEST_COMPARATOR;
        entries: number;
        bytes: number;
        manifestSha256: string;
    ...
    sourceManifestHelper: IFileSeal;
...
function commandLine(): ICli {
    ...
    if (values.help) {
        console.log(
            "Usage: bun docs/evidence/tools/a13_stat_rounding_near_grid_v6_micro.ts " +
                "--attempt-id=UUID --out=REPORT.json [--smoke] | --source-preflight-only",
        );
        process.exit(0);
    }
    if (
...
function directorySeal(pathInput: string): IDirectorySeal {
    ...
    return {
        ...
        manifestCodec: MICRO_RUNTIME_DEPENDENCY_MANIFEST_CODEC,
...
function runSeal(): IRunSeal {
    ...
    const sourceManifestHelper = fileSeal(SOURCE_MANIFEST_HELPER_PATH);
    ...
    const sealWithoutIdentity = {
        ...
        source: {
            manifestSchema: source.schema,
            manifestPathScope: source.pathScope,
            manifestComparator: source.comparator,
        ...
        sourceManifestHelper: {
            ...sourceManifestHelper,
            path: normalizedPath(relative(ROOT, SOURCE_MANIFEST_HELPER_PATH)),
        },
...
function assertExpectedRuntimeInputs(seal: IRunSeal, context: string): Record<string, unknown> {
    ...
    for (const name of RUNTIME_DEPENDENCY_NAMES) {
        ...
        if (
            actual.manifestCodec !== MICRO_RUNTIME_DEPENDENCY_MANIFEST_CODEC ||
...
function assertExpectedSourceIdentity(seal: IRunSeal, context: string): Record<string, unknown> {
    ...
    assertA13NearGridSourceManifestIdentity(
        {
            schema: seal.source.manifestSchema,
            pathScope: seal.source.manifestPathScope,
            comparator: seal.source.manifestComparator,
            entryCount: seal.source.entries,
            bytes: seal.source.bytes,
            manifestSha256: seal.source.manifestSha256,
        },
        {
            ...
            entryCount: EXPECTED_SOURCE_ENTRIES,
    ...
    return {
        ...
        stagedSourceDelta: [],
        sourceStatus: [],
        expectedSrcManifestSchema: A13_NEAR_GRID_SOURCE_MANIFEST_SCHEMA,
        observedSrcManifestSchema: seal.source.manifestSchema,
        expectedSrcManifestPathScope: A13_NEAR_GRID_SOURCE_MANIFEST_PATH_SCOPE,
        observedSrcManifestPathScope: seal.source.manifestPathScope,
        expectedSrcManifestComparator: A13_NEAR_GRID_SOURCE_MANIFEST_COMPARATOR,
        observedSrcManifestComparator: seal.source.manifestComparator,
        expectedSrcEntries: EXPECTED_SOURCE_ENTRIES,
        observedSrcEntries: seal.source.entries,
        expectedSrcBytes: EXPECTED_SOURCE_BYTES,
        observedSrcBytes: seal.source.bytes,
...
async function main(): Promise<void> {
    if (process.argv.includes("--source-preflight-only")) {
        if (process.argv.length !== 3 || process.argv[2] !== "--source-preflight-only") {
            throw new Error("--source-preflight-only must be the sole argument");
        }
        const runtimeInjection = auditRuntimeInjection();
        const bunConfiguration = auditBunConfigFiles();
        const seal = runSeal();
        const governedSource = assertExpectedSourceIdentity(seal, "source-preflight");
        const governedRuntimeInputs = assertExpectedRuntimeInputs(seal, "source-preflight");
        const stableGovernedSource = { ...governedSource };
        delete stableGovernedSource.currentHead;
        console.log(
            JSON.stringify(
                {
                    ...
                    nonEmpirical: true,
                    noAttemptId: true,
                    noOutputPathOrArtifactWrite: true,
                    noCandidateImportCorrectnessWarmupCalibrationOrTiming: true,
                    runtimeInjection,
                    bunConfiguration,
                    source: stableGovernedSource,
                    runtimeInputs: governedRuntimeInputs,
                    runner: seal.runner,
                    sourceManifestHelper: seal.sourceManifestHelper,
                    sourceSeal: seal.source,
                },
                null,
                2,
            ),
