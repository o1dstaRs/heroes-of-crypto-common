 ...
 *
 * Evidence:
 *   bun docs/evidence/tools/a13_stat_rounding_micro.ts \
 *     --out=/tmp/a13-stat-rounding-micro.json
 *
 * Short performance smoke (the full deterministic correctness corpus still runs):
 *   bun docs/evidence/tools/a13_stat_rounding_micro.ts \
 *     --smoke --out=/tmp/a13-stat-rounding-micro-smoke.json
 *
...
const SCHEMA = "heroes-of-crypto/a13-stat-rounding-micro/v3" as const;
...
const EXPECTED_CANDIDATE_COMMIT = "4d8d94a65aeb77ca953e2ee410b40eaa81d236d8";
const EXPECTED_SOURCE_MANIFEST_SHA256 = "86ba3f5d3cb5ae74cca97ec21fd5031b43094975337a3ae12f2c2f2c223361a0";
...
const MICRO_MANIFEST_CODEC = "heroes-of-crypto/a13-stat-rounding-micro-manifest/type-tagged-v1" as const;
...
function commandLine(): ICli {
    ...
    if (values.help) {
        console.log(
            "Usage: bun docs/evidence/tools/a13_stat_rounding_micro.ts " +
...
function rawFloat64Correctness(candidate: Round): Record<string, unknown> {
    ...
    for (let index = 0; index < RAW_FLOAT64_PATTERNS; index++) {
        ...
        for (const fractionDigits of [1, 2] as const) {
            ...
            if (isFastEligible(value, fractionDigits)) fastEligible++;
...
function curatedCorrectness(candidate: Round): Record<string, unknown> {
    ...
    for (let scaled = -100_000; scaled <= 100_000; scaled += 37) {
        for (const fractionDigits of [1, 2] as const) {
            ...
            for (const neighbor of [nextDown(value), value, nextUp(value)]) {
                ...
                gridComparisons++;
                if (isFastEligible(neighbor, fractionDigits)) gridFastEligible++;
            }
        }
    }
    ...
    return {
        ...
        decimalGrids: {
            ...
            independentlyCalculatedFastEligible: gridFastEligible,
        },
...
async function captureActualStatTrace(
    ...
    const fastEligible = cases.filter((item) => isFastEligible(item.value, item.fractionDigits)).length;
    const digitCounts = {
...
function quantile(values: readonly number[], probability: number): number {
    ...
    return finitePositive(sorted[lower] * (1 - fraction) + sorted[upper] * fraction, `ratio quantile ${probability}`);
}
