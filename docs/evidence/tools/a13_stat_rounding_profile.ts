...
const SCHEMA = "heroes-of-crypto/a13-stat-rounding-profile/v2" as const;
const CAPTURE_SCHEMA = "heroes-of-crypto/a13-stat-rounding-capture/v2" as const;
const TELEMETRY_SCHEMA = "heroes-of-crypto/a13-stat-rounding-telemetry/v2" as const;
...
const CANDIDATE_SRC_MANIFEST_SHA256 = "033f499edbefb61491d1c49b8f49be7b480f7b0592795eca6dee207ba16dbdfa";
...
export function roundUnitStat(value: number, fractionDigits: UnitStatFractionDigits): number {
    ...
    let classification: "fast" | "numericFallback" | "dynamicFallback";
    if (
        typeof value !== "number" ||
...
async function telemetryMain(args: string[]): Promise<void> {
    ...
    Bun.plugin({
        name: `a13-stat-rounding-telemetry-${variant}-${process.pid}`,
        setup(build): void {
...
async function orchestrate(args: string[]): Promise<void> {
    ...
    if (values.help) {
        console.log(
            "Usage: bun docs/evidence/tools/a13_stat_rounding_profile.ts " +
    ...
    const temporaryRoot = mkdtempSync(join(tmpdir(), "hoc-a13-stat-rounding-profile-"));
    ...
    try {
        ...
        const telemetryClassificationPassed =
            candidateCounts.fast + candidateCounts.numericFallback + candidateCounts.dynamicFallback ===
                ...
                attribution: {
                    ...
                    timingExcludedTelemetry: {
                        ...
                        requirements: [
                            ...
                            "oracleChecks === calls",
                            "mismatches === 0",
                            "fast + numericFallback + dynamicFallback === calls",
                            "fast / calls >= 0.90",
            ...
            gates: {
                ...
                timingExcludedTelemetry: {
                    ...
                    classification: {
                        ...
                        sum: candidateCounts.fast + candidateCounts.numericFallback + candidateCounts.dynamicFallback,
