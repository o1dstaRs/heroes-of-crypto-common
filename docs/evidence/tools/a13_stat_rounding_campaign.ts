...
const SCHEMA = "heroes-of-crypto/a13-stat-rounding-campaign/v1" as const;
...
const PAIR_SCHEMA = "heroes-of-crypto/a13-stat-rounding-pair/v2" as const;
...
const REPLICATION_SCHEMA = "heroes-of-crypto/a13-stat-rounding-replication/v2" as const;
const CAMPAIGN_ID = "a13-stat-rounding-4d8d94a-20260723-v2";
...
const PAIR_PATH = resolve(dirname(RUNNER_PATH), "a13_stat_rounding_pair.ts");
const MICRO_PATH = resolve(dirname(RUNNER_PATH), "a13_stat_rounding_micro.ts");
const PROFILE_PATH = resolve(dirname(RUNNER_PATH), "a13_stat_rounding_profile.ts");
const REPLICATION_PATH = resolve(dirname(RUNNER_PATH), "a13_stat_rounding_replication.ts");
...
interface IHostAttestation {
    schema: "heroes-of-crypto/a13-stat-rounding-host-attestation/v1";
    observedAt: string;
    identity: {
        hostname: string;
        platform: string;
...
interface IHostHealthSample {
    schema: "heroes-of-crypto/a13-stat-rounding-host-health/v1";
    observedAt: string;
    power: { ac: true; rawSha256: string; raw: string };
    thermal: { nominal: true; rawSha256: string; raw: string };
    overlap: { passed: true; markers: readonly string[]; matchingProcesses: [] };
...
function assertFrozenPrelude(
    ...
    const governedFiles = [protocolSeal, ...Object.values(expectedRunners).map(({ path }) => fileSeal(path))];
...
async function main(): Promise<void> {
    ...
    const aggregateOutcome = await new Promise<{ code: number | null; signal: NodeJS.Signals | null }>(
        (resolvePromise, reject) => {
            ...
            aggregate.once("exit", (code, signal) => resolvePromise({ code, signal }));
        },
    );
    const aggregateConfigurationAfter = auditConfigurationAbsence(aggregateHome);
    assertJsonEqual(
    ...
    auditRuntimeInjection();
    if (aggregateOutcome.code !== 0 || aggregateOutcome.signal !== null || !existsSync(aggregatePath)) {
        throw new Error(
            `Aggregation failed after ledger closure ${closure.sha256}: ` +
                `code=${aggregateOutcome.code} signal=${aggregateOutcome.signal}`,
        );
    }
    console.log(
        JSON.stringify({
            schema: SCHEMA,
            campaignId: CAMPAIGN_ID,
            aggregate: fileSeal(aggregatePath),
