...
const SCHEMA = "heroes-of-crypto/a13-stat-rounding-near-grid-campaign/v2" as const;
...
const REPLICATION_SCHEMA = "heroes-of-crypto/a13-stat-rounding-near-grid-replication/v3" as const;
const CAMPAIGN_ID = "a13-stat-rounding-near-grid-77ee4616-20260723-v5";
...
const PAIR_PATH = resolve(dirname(RUNNER_PATH), "a13_stat_rounding_near_grid_v5_pair.ts");
const MICRO_PATH = resolve(dirname(RUNNER_PATH), "a13_stat_rounding_near_grid_v5_micro.ts");
const PROFILE_PATH = resolve(dirname(RUNNER_PATH), "a13_stat_rounding_near_grid_v5_profile.ts");
const REPLICATION_PATH = resolve(dirname(RUNNER_PATH), "a13_stat_rounding_near_grid_v5_replication.ts");
...
const BASELINE_ROOT = "/private/tmp/hoc-stat-rounding-near-grid-v5-qualification.eU9OEp/baseline";
const CANDIDATE_ROOT = "/private/tmp/hoc-stat-rounding-near-grid-v5-qualification.eU9OEp/candidate";
...
const SOURCE_DEPENDENCY_PREFLIGHT_SCHEMA =
    "heroes-of-crypto/a13-stat-rounding-near-grid-source-dependency-preflight/v1";
const V4_INCIDENT_SCHEMA = "heroes-of-crypto/a13-stat-rounding-near-grid-v4-infrastructure-incident/v1";
...
function assertSourceAndDependencyPreflight(baselineRoot: string, candidateRoot: string): void {
    if (baselineRoot !== BASELINE_ROOT || candidateRoot !== CANDIDATE_ROOT) {
        throw new Error("Source/dependency preflight roots differ from the preregistered fresh v5 extraction");
    }
    ...
    for (const [label, root, archiveSha256, srcManifestSha256] of [
        ...
        const source = pairTreeSeal(join(root, "src"));
...
function expectedV4IncidentBinding(): Record<string, unknown> {
    const incident = requireRecord(JSON.parse(readFileSync(INCIDENT_PATH, "utf8")), "v4 incident");
    if (incident.schema !== V4_INCIDENT_SCHEMA) throw new Error("V4 infrastructure incident schema mismatch");
    const seal = fileSeal(INCIDENT_PATH);
    return {
        schema: V4_INCIDENT_SCHEMA,
        incident: {
...
function assertFrozenPrelude(
    ...
    if (
        ...
        !jsonEqual(freeze.v4InfrastructureIncident, fileSeal(INCIDENT_PATH))
    ) {
    ...
    assertJsonEqual(
        protocol.v4InfrastructureIncident,
        expectedV4IncidentBinding(),
