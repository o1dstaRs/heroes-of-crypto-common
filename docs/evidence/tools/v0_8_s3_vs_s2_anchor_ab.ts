...
/**
 * External, source-sealed v0.8+a13 shortlist-3 versus shortlist-2 strength harness.
 *
 * This file intentionally lives outside src/. It does not patch SearchDriver or add a
 * production experiment seam. The candidate arm uses runMatch's untouched automatic
 * v0.8+a13 factory. The control arm uses the canonical a13 environment with exactly one
 * change: SEARCH_SHORTLIST=2. Each arm fights the same fixed v0.7 anchor in a complete
 * roster-by-physical-seat crossover, on the same setup/map/combat seed.
 *
 * No-fight checks:
 ...
 *     --stage smoke --concurrency 6 --output /tmp/hoc-s3-s2-smoke
 */
...
const SCHEMA = "hoc.v0_8_s3_vs_s2_anchor_ab.v1" as const;
const CLUSTER_SIZE_PER_ARM = 4 as const;
const GAMES_PER_CLUSTER = 8 as const;
const CANDIDATE_VERSION = "v0.8" as const;
const ANCHOR_VERSION = "v0.7" as const;
const PRODUCTION_SHORTLIST = 3 as const;
const CONTROL_SHORTLIST = 2 as const;
const RAW_FILE = "s3-vs-s2-anchor-ab.records.jsonl";
const SUMMARY_FILE = "s3-vs-s2-anchor-ab.summary.json";
const STARTED_FILE = "s3-vs-s2-anchor-ab.started.json";
const VALIDATION_SEED_LABEL = "hoc.s3-s2.validation-seed.v1";
const VALIDATION_SEED = 386_914_648;
const VALIDATION_SEED_COMMITMENT = "44927f377bd11625d6df38075f7d72dc220916140156c1de1848ba390f64f9cb";
...
type Stage = "smoke" | "direction" | "development" | "validation" | "replication";
type Arm = "s3" | "s2";
type CandidateRoster = "a" | "b";

const STAGE_PAIRS: Readonly<Record<Stage, number>> = Object.freeze({
    smoke: 36,
    direction: 360,
    development: 1_800,
    validation: 3_600,
...
const STAGE_SEEDS: Readonly<Record<Stage, number>> = Object.freeze({
    smoke: 1_391_574_133,
    direction: 1_618_033_989,
    development: 1_391_574_133,
    validation: VALIDATION_SEED,
    replication: 1_278_621_871,
});

const GATES = Object.freeze({
    practicalLiftPp: 1,
    clusteredCiLowPp: 0.25,
    safetyFloorPp: -2,
});
...
interface IClusterRecord {
    ...
    map: number;
    armOrder: [Arm, Arm];
    games: [IGameRecord, IGameRecord, IGameRecord, IGameRecord, IGameRecord, IGameRecord, IGameRecord, IGameRecord];
}
...
interface IEnvironmentSeal {
    productionShortlist: number;
    productionGenomeSha256: string;
    productionBehaviorEnvironmentSha256: string;
    candidateProfileSha256: string;
    controlProfileSha256: string;
    profileDelta: { key: string; candidate: string | null; control: string | null }[];
    validationSeedCommitment: string;
}

interface ISourceRuntime {
    profile: typeof import("../../../src/ai/versions/v0_8_a13_profile");
...
async function loadSourceRuntime(sourceRoot: string): Promise<ISourceRuntime> {
    ...
    const [profile, army, battle, cohorts, search] = await Promise.all([
    ...
    return { profile, army, battle, cohorts, search } as ISourceRuntime;
}

const candidateProfileEnvironment = (runtime: ISourceRuntime): Readonly<Record<string, string | undefined>> =>
    runtime.profile.buildV08A13SearchEnvironment(CANDIDATE_VERSION);

const controlProfileEnvironment = (runtime: ISourceRuntime): Readonly<Record<string, string | undefined>> => ({
    ...candidateProfileEnvironment(runtime),
    SEARCH_SHORTLIST: String(CONTROL_SHORTLIST),
});

function profileDelta(runtime: ISourceRuntime): { key: string; candidate: string | null; control: string | null }[] {
    const candidate = new Map(stableEntries(candidateProfileEnvironment(runtime)));
    const control = new Map(stableEntries(controlProfileEnvironment(runtime)));
    return [...new Set([...candidate.keys(), ...control.keys()])].sort().flatMap((key) => {
        const candidateValue = candidate.get(key) ?? null;
        const controlValue = control.get(key) ?? null;
        return candidateValue === controlValue ? [] : [{ key, candidate: candidateValue, control: controlValue }];
    });
}

function validationSeedCommitment(seed: number): string {
    return sha256(`${VALIDATION_SEED_LABEL}\0${seed}`);
}
...
function assertEnvironmentContract(runtime: ISourceRuntime): IEnvironmentSeal {
    ...
    const controlEnvironment = controlProfileEnvironment(runtime);
    ...
    if (runtime.profile.V08_A13_SEARCH.shortlist !== PRODUCTION_SHORTLIST) {
        throw new Error(
            `This harness requires production a13 shortlist=${PRODUCTION_SHORTLIST}; imported ${runtime.profile.V08_A13_SEARCH.shortlist}`,
        );
    }
    if (candidateEnvironment.V07_SEARCH !== "1") {
        throw new Error("Canonical a13 environment must explicitly enable V07_SEARCH=1");
    }
    if (candidateEnvironment.SEARCH_SHORTLIST !== String(PRODUCTION_SHORTLIST)) {
        throw new Error("Canonical a13 environment does not match the imported production shortlist");
    }
    if (
        delta.length !== 1 ||
        delta[0].key !== "SEARCH_SHORTLIST" ||
        delta[0].candidate !== String(PRODUCTION_SHORTLIST) ||
        delta[0].control !== String(CONTROL_SHORTLIST)
    ) {
        throw new Error(`S3/S2 environment delta escaped SEARCH_SHORTLIST: ${JSON.stringify(delta)}`);
    }
    if (validationSeedCommitment(VALIDATION_SEED) !== VALIDATION_SEED_COMMITMENT) {
        throw new Error("Embedded validation seed does not match its preregistered commitment");
    }
    return {
        productionShortlist: runtime.profile.V08_A13_SEARCH.shortlist,
        productionGenomeSha256: runtime.profile.V08_A13_GENOME_SHA256,
        productionBehaviorEnvironmentSha256: runtime.profile.V08_A13_PRODUCTION_BEHAVIOR_ENVIRONMENT_SHA256,
        candidateProfileSha256: environmentSha256(candidateEnvironment),
        controlProfileSha256: environmentSha256(controlEnvironment),
        profileDelta: delta,
        validationSeedCommitment: VALIDATION_SEED_COMMITMENT,
    };
}

function parseCohorts(raw: string | undefined): AiMetaCohort[] {
    const values = raw
        ? raw
              .split(",")
...
function parseOptions(argv: readonly string[]): IOptions {
    ...
    const stage = (values.stage ?? "smoke") as Stage;
    if (!(stage in STAGE_PAIRS))
        throw new Error("stage must be smoke, direction, development, validation, or replication");
    const pairs = positiveInteger(values.pairs, "pairs", STAGE_PAIRS[stage]);
    if (pairs !== STAGE_PAIRS[stage] || pairs % 36 !== 0) {
        throw new Error(`${stage} requires exactly ${STAGE_PAIRS[stage]} pairs per cohort, divisible by 36`);
    }
    const baseSeed = Number(values.seed ?? STAGE_SEEDS[stage]);
    if (!Number.isSafeInteger(baseSeed) || baseSeed !== STAGE_SEEDS[stage]) {
        throw new Error(`${stage} requires preregistered seed ${STAGE_SEEDS[stage]}`);
    }
    ...
    if (stage === "direction" || stage === "validation" || stage === "replication") {
        if (maxLaps !== 60) throw new Error(`${stage} requires max-laps 60`);
        if (cohorts.length !== AI_META_COHORTS.length || AI_META_COHORTS.some((cohort) => !cohorts.includes(cohort))) {
            throw new Error(`${stage} requires all preregistered cohorts`);
        }
        if (!values["expected-source-sha256"]?.trim()) {
            throw new Error(`${stage} requires --expected-source-sha256 to bind the sealed source archive`);
        }
    }
    ...
    return {
        ...
        sourceRoot: resolve(values["source-root"] ?? DEFAULT_SOURCE_ROOT),
...
function sourceSeal(sourceRoot: string): ISourceSeal {
    ...
    return {
        ...
        files: files.length,
        bytes,
        profileSha256: fileSha256(join(sourceRoot, "src/ai/versions/v0_8_a13_profile.ts")),
...
function workerEnvironment(sourceRoot: string): Record<string, string> {
    const environment: Record<string, string> = {
        ...
        HOC_S3_S2_SOURCE_ROOT: sourceRoot,
    };
...
function behaviorEnvironment(): Record<string, string | undefined> {
    ...
    const exact = new Set(["NODE_ENV", "SIM_NO_ACTIONS", "LIVETWIN", "FIGHT_MELEE_ROSTERS", "HOC_S3_S2_SOURCE_ROOT"]);
...
function playGame(
    ...
    // Candidate S3 deliberately takes runMatch's ordinary automatic production factory. S2 supplies the
    // canonical profile to the ordinary generic SearchDriver, changing only SEARCH_SHORTLIST.
    const result =
        arm === "s3"
            ? runtime.battle.runMatch(config)
            : runtime.search.withScopedAIEnvironment(controlProfileEnvironment(runtime), () =>
                  runtime.battle.runMatch(config),
              );
        ...
        for (let index = 0; index < workerCount; index += 1) {
            ...
            worker.on("message", (message: WorkerResponse) => {
                ...
                if (now - lastProgressAt >= 5_000 || completed === tasks.length) {
                    lastProgressAt = now;
                    console.error(
                        `[s3-s2] ${completed}/${tasks.length} clusters (${completed * GAMES_PER_CLUSTER} fights)`,
                    );
                }
            ...
            worker.on("exit", (code) => {
                if (!settled && code !== 0) fail(new Error(`S3/S2 worker exited with code ${code}`));
            });
        }
    });
}
...
async function workerMain(workerPort: NonNullable<typeof parentPort>): Promise<void> {
    const sourceRoot = process.env.HOC_S3_S2_SOURCE_ROOT;
    if (!sourceRoot) throw new Error("S3/S2 worker requires HOC_S3_S2_SOURCE_ROOT");
    const runtime = await loadSourceRuntime(sourceRoot);
...
export {
    SCHEMA as V08_S3_VS_S2_ANCHOR_AB_SCHEMA,
