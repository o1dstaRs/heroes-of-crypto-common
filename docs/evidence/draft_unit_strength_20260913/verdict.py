#!/usr/bin/env python3
"""Apply the PREREGISTRATION.md gates to the confirmation and robustness outputs."""
import json
import os
...
confirm = load("confirm_vs_live")
head = opponent(confirm, "ranked-live-incumbent")
...
candidate_ref = load("robust_candidate_vs_reference")
incumbent_ref = load("robust_incumbent_vs_reference")
...
drafts = {}
with open(f"{OUT}/confirm_vs_live.jsonl") as handle:
    for line in handle:
...
for name, value, passed in gates:
    print(f"{'PASS' if passed else 'FAIL'}  {name}: {value:.4f}" if isinstance(value, float) else f"{'PASS' if passed else 'FAIL'}  {name}: {value}")
