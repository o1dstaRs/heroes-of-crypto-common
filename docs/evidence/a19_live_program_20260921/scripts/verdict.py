...
Usage: VERDICT_OUT=<run dir> verdict.py <confirm_name> <robust_candidate_name> <robust_incumbent_name> <incumbent_id>
...
OUT = os.environ.get("VERDICT_OUT") or sys.exit("set VERDICT_OUT to the directory holding the run's .json/.jsonl files")
# The fitted v1 prior ships in common and carries each creature's faction (used for the tier-2 reach line).
PRIOR = os.path.join(os.path.dirname(os.path.abspath(__file__)), *[".."] * 4,
                     "src", "ai", "setup", "draft_strength_priors", "ranked_unit_strength_a19_side_v1.json")
